    ...
    <div className="space-y-6">
        ...
        >
          <div>
            <h3 className={`font-semibold text-base ${isDark ? 'text-slate-50' : 'text-slate-900'}`}>
              ข้อมูลองค์กรและหัวเอกสาร (Company & Invoice Header)
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            ...
            <div className="space-y-1.5">
              <label className="font-semibold text-slate-700 dark:text-slate-300">เลขประจำตัวผู้เสียภาษี (Tax ID)</label>
              <input
                type="text"
            ...
            <div className="space-y-1.5">
              <label className="font-semibold text-slate-700 dark:text-slate-300">อีเมลทางการ</label>
              <input
                type="email"
            ...
            <div className="md:col-span-2 space-y-1.5">
              <label className="font-semibold text-slate-700 dark:text-slate-300">ที่อยู่สำนักงาน / คลังสินค้าหลัก</label>
              <textarea
                rows={2}
      ...
      {activeTab === 'inventory' && (
        ...
        >
          <div>
            <h3 className={`font-semibold text-base ${isDark ? 'text-slate-50' : 'text-slate-900'}`}>
              นโยบายการตัดสต็อกและค่าเริ่มต้น (Inventory Engine Defaults)
            </h3>
            <p className={`text-xs mt-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {[
                  { value: 'FIFO', title: 'FIFO (First-In, First-Out)', desc: 'ตัดล็อตที่นำเข้าก่อนเป็นลำดับแรก (แนะนำสำหรับสินค้าทั่วไป)' },
                  { value: 'FEFO', title: 'FEFO (First-Expired, First-Out)', desc: 'ตัดล็อตที่หมดอายุก่อนเป็นลำดับแรก (สำหรับยา/อาหาร)' },
                  { value: 'MANUAL', title: 'Manual Selection', desc: 'ให้เจ้าหน้าที่เลือกหมายเลข Lot/Bin ด้วยตนเอง' },
                ].map((item) => (
                  <label
                    key={item.value}
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="font-semibold text-slate-700 dark:text-slate-300">หน่วยนับเริ่มต้น (Default UOM)</label>
                <input
                  type="text"
              ...
              <div className="space-y-1.5">
                <label className="font-semibold text-slate-700 dark:text-slate-300">มาตรฐานบาร์โค้ดหลัก</label>
                <select
                  value={settings.defaultBarcodeSymbology}
                  onChange={(e) => setSettings({ ...settings, defaultBarcodeSymbology: e.target.value as any })}
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-600 outline-hidden font-medium"
                >
                  <option value="CODE128">CODE128 (รองรับตัวอักษรและตัวเลข - สากล)</option>
                  <option value="EAN13">EAN13 (รหัสสินค้า 13 หลัก)</option>
                  <option value="QR_CODE">QR CODE (2D Matrix Code)</option>
                </select>
              </div>
      ...
      {activeTab === 'alerts' && (
        ...
        >
          <div>
            <h3 className={`font-semibold text-base ${isDark ? 'text-slate-50' : 'text-slate-900'}`}>
              กฎการแจ้งเตือนและความปลอดภัยของสินค้า (Alert Thresholds)
            </h3>
            <p className={`text-xs mt-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
      ...
      {activeTab === 'security' && (
        ...
        >
          <div>
            <h3 className={`font-semibold text-base ${isDark ? 'text-slate-50' : 'text-slate-900'}`}>
              ความปลอดภัยและการสำรองข้อมูล (Security & Master Data Backup)
            </h3>
            <p className={`text-xs mt-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
            ...
            <div className={`p-5 rounded-2xl border space-y-3 ${isDark ? 'border-slate-800 bg-slate-800/40' : 'border-slate-200 bg-slate-50'}`}>
              ...
              >
                ...
                <span>{isExporting ? 'กำลังส่งออก...' : t.exportBackup}</span>
              </button>
            </div>
          </div>
        </div>
