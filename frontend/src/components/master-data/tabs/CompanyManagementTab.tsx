...
export const CompanyManagementTab: React.FC<CompanyManagementTabProps> = ({
  ...
  const hqCount = safeCompanies.filter((c) => c.isHeadquarter).length;
  const branchCount = safeCompanies.filter((c) => !c.isHeadquarter).length;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[14px] text-slate-400 font-medium">
                {isEn ? 'Headquarters' : 'สำนักงานใหญ่ (Headquarters)'}
              </p>
              <h4 className="text-[22px] font-bold mt-0.5">
                {isEn
                  ? `${hqCount} ${hqCount === 1 ? 'HQ' : 'HQs'}`
                  : `${hqCount} แห่ง`}
              </h4>
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[14px] text-slate-400 font-medium">
                {isEn ? 'Branches' : 'สาขาย่อย (Branches)'}
              </p>
              <h4 className="text-[22px] font-bold mt-0.5">
                {isEn
          ...
          {filtered.map((company) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {company.isHeadquarter ? (
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/30">
                          {isEn ? '★ Headquarters' : '★ สำนักงานใหญ่'}
                        </span>
                      ) : (
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-slate-100 dark:bg-slate-800 text-slate-500 border border-slate-200 dark:border-slate-700">
                          {isEn ? 'Branch' : 'สาขา'} {company.branchCode}
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-slate-400 font-mono mt-0.5">
                      {company.code} • {isEn ? 'Branch' : 'สาขา'}: {company.branchName || company.branchCode}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    title={isEn ? 'View Company Details' : 'ดูรายละเอียดบริษัท (View Detail)'}
                  >
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/80 grid grid-cols-2 gap-3 text-xs">
                ...
                <div>
                  <p className="text-slate-400 font-medium">
                    {isEn ? 'Contact (Phone / Email)' : 'ติดต่อ (Phone / Email)'}
                  </p>
              ...
              {company.address && (
                <div className="mt-2.5 pt-2 border-t border-slate-100 dark:border-slate-800/50 text-xs">
                  <p className="text-slate-400 font-medium">
                    {isEn ? 'Legal Address:' : 'ที่อยู่สถานประกอบการ:'}
                  </p>
